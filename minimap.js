...
export class Minimap {
  ...
  draw(point,yaw,route,destination){
    ...
    const nodes=new Map(this.project.network.nodes.map(n=>[n.id,n]));ctx.strokeStyle='#739e9b';ctx.lineWidth=2;ctx.beginPath();for(const e of this.project.network.edges){const a=plot(nodes.get(e.a).position),b=plot(nodes.get(e.b).position);ctx.moveTo(...a);ctx.lineTo(...b);}ctx.stroke();
    if(route){ctx.beginPath();route.points.forEach((p,i)=>{const q=plot(p);i?ctx.lineTo(...q):ctx.moveTo(...q);});ctx.lineWidth=4;ctx.strokeStyle='#00858b';ctx.stroke();}
    for(const p of this.project.places){if(!p.visible)continue;const q=plot(p.position);ctx.beginPath();ctx.arc(...q,p.id===destination?.id?5:3,0,Math.PI*2);ctx.fillStyle=p.color;ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=1;ctx.stroke();}

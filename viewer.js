...
export class Viewer {
  constructor(container) {
    ...
    this.renderer.shadowMap.enabled=false; // Lighting owns the current shadow projection and its transparent casters.
  ...
  setModel(model){
    ...
    // One receiving floor avoids competing almost-coplanar surfaces at distance.
    this.ground=new THREE.Mesh(new THREE.PlaneGeometry(this.span*180,this.span*180),this.makeMaterial(1,0xf7fafb));this.ground.rotation.x=-Math.PI/2;this.ground.position.set(this.center.x,this.groundY,this.center.z);this.scene.add(this.ground);
    this.scene.fog=new THREE.Fog(0xe5eef1,this.span*8,this.span*55);
    this.camera.far=this.span*90;this.camera.near=Math.max(.002,this.span*.001);this.camera.updateProjectionMatrix();this.controls.maxDistance=this.span*5;this.controls.minDistance=this.span*.08;
  ...
  render(){this.lighting.beforeRender();this.renderer.render(this.scene,this.camera);this.dirty=false;}
}

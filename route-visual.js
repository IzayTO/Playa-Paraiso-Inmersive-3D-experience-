...
import {pointOnPath,clamp,distance} from './math.js?v=1.4';
export class RouteVisual {
  constructor(viewer){this.viewer=viewer;this.group=new THREE.Group();viewer.scene.add(this.group);this.route=null;this.style='line';this.progress=0;}
  setStyle(style){this.style=style==='breadcrumbs'?'breadcrumbs':'line';if(this.uniforms)this.uniforms.breadcrumbs.value=this.style==='breadcrumbs'?1:0;this.viewer.dirty=true;}
  setProgress(progress){this.progress=clamp(progress,0,1);if(this.uniforms)this.uniforms.progress.value=this.progress;this.viewer.dirty=true;}
  ...
  show(route){
    this.clear();this.progress=0;this.route=route;this.started=performance.now();this.reduced=matchMedia('(prefers-reduced-motion:reduce)').matches;
    ...
    this.uniforms={reveal:{value:0},time:{value:0},total:{value:covered},spacing:{value:width*13},progress:{value:0},breadcrumbs:{value:this.style==='breadcrumbs'?1:0}};
    ...
    const material=new THREE.ShaderMaterial({uniforms:this.uniforms,side:THREE.DoubleSide,transparent:true,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-2,polygonOffsetUnits:-2,
      ...
      fragmentShader:`varying vec2 vUV;uniform float reveal;uniform float time;uniform float total;uniform float spacing;uniform float progress;uniform float breadcrumbs;
      ...
      vec3 col=mix(vec3(0.0,.42,.46),vec3(.91,1.0,.98),arrow*.93);float alpha=edge*.97;
      if(breadcrumbs>.5&&vUV.x>progress*total){
        float cell=fract(vUV.x/spacing),chevron=abs(cell-(.60-abs(vUV.y)*.18));
        float mark=(1.0-smoothstep(.035,.065,chevron))*(1.0-smoothstep(.58,.84,abs(vUV.y)));
        if(mark<.02)discard;col=mix(vec3(0.0,.44,.48),vec3(.43,.84,.80),.25+.15*sin(vUV.x/spacing*1.2-time/spacing));alpha=mark*.96;

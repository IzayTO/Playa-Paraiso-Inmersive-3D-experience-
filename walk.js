...
import {clamp,pointOnPath,directionCue} from './math.js?v=1.1-flat';
export class WalkController {
  constructor(viewer,onChange){
    ...
    canvas.addEventListener('pointerdown',e=>{if(!this.active||e.button!==0)return;this.viewer.container.focus({preventScroll:true});this.drag={id:e.pointerId,x:e.clientX,y:e.clientY};canvas.setPointerCapture(e.pointerId);e.preventDefault();});
  ...
  start(route,from,to){
    ...
    this.saved={position:this.viewer.camera.position.clone(),target:this.viewer.controls.target.clone(),near:this.viewer.camera.near};this.route=route;this.from=from;this.to=to;this.position=0;this.eye=this.viewer.baseEyeHeight;this.pitch=0;
    const [a,b]=route.points;this.yaw=Math.atan2(b[0]-a[0],-(b[2]-a[2]));
    this.active=true;this.viewer.walking=true;this.viewer.transition=null;this.viewer.controls.enabled=false;this.viewer.camera.fov=70;this.viewer.camera.near=Math.max(.001,this.eye*.05);this.viewer.resize();this.resetInput();this.viewer.container.focus({preventScroll:true});this.update(0);
  }
  stop(){
    if(!this.active)return;this.active=false;this.viewer.walking=false;this.resetInput();this.viewer.camera.fov=43;this.viewer.camera.near=this.saved.near;this.viewer.camera.position.copy(this.saved.position);this.viewer.controls.target.copy(this.saved.target);this.viewer.controls.enabled=true;this.viewer.resize();this.viewer.controls.update();
  }
